...
class SubtractBackgroundConfig(pexConfig.Config):
    ...
    """
    statisticsProperty = ccf.ChoiceField(
    ...
    undersampleStyle = ccf.ChoiceField(
...
class SubtractBackgroundTask(Task):
    ...
    def fitBackground(self, maskedImage, nx=0, ny=0, algorithm=None):
        ...
        if (maskedImage.mask.getArray() & badMask).all():
            raise TaskError("All pixels masked. Cannot estimate background")
