...
public:
    ...
    std::pair<double, double> getThroughputAtBounds() const override { return std::make_pair(1.0, 1.0); }

    void sampleAt(lsst::geom::Point2D const&, ndarray::Array<double const, 1, 1> const& wavelengths,
...
protected:
    ...
    std::shared_ptr<TransmissionCurve const> _transformedByImpl(
            std::shared_ptr<geom::TransformPoint2ToPoint2> transform) const override {
    ...
    std::shared_ptr<TransmissionCurve const> _multipliedByImpl(
            std::shared_ptr<TransmissionCurve const> other) const override {
        return other;
    }

    std::string getPersistenceName() const override { return NAME; }

    void write(OutputArchiveHandle& handle) const override { handle.saveEmpty(); }
...
public:
    ...
    Impl1d(ndarray::Array<double, 1, 1> const& throughput, ndarray::Array<double, 1, 1> const& wavelengths)
    ...
    static void setupPersistence(table::Schema& schema, ArrayKeyVector& keys) {
        keys.push_back(schema.addField<table::Array<double>>("throughput", "array of known throughput values",
                                                             "", 0));
        keys.push_back(schema.addField<table::Array<double>>(
    ...
    struct Functor {
        ...
        double operator()(Impl1d const& parent, double wavelength) {
            ...
            int status = ::gsl_interp_eval_e(parent._interp.get(), parent._wavelengths.getData(),
                                             parent._throughput.getData(), wavelength, _accel.get(), &result);
...
public:
    ...
    Impl2d(ndarray::Array<double, 1, 1> const& throughput, ndarray::Array<double, 1, 1> const& wavelengths,
           ndarray::Array<double, 1, 1> const& radii)
            : _throughput(throughput),
              _wavelengths(wavelengths),
              _radii(radii),
              _interp(makeGslPtr(
                      ::gsl_interp2d_alloc(::gsl_interp2d_bilinear, _wavelengths.size(), _radii.size()),
                      &::gsl_interp2d_free)) {
    ...
    static void setupPersistence(table::Schema& schema, ArrayKeyVector& keys) {
        ...
        keys.push_back(schema.addField<table::Array<double>>(
                "wavelengths", "array of known wavelength values", "angstrom", 0));
        keys.push_back(schema.addField<table::Array<double>>("radii", "array of known radius values", "", 0));
    }
    ...
    struct Functor {
        ...
        double operator()(Impl2d const& parent, double wavelength) {
            double result = 0.0;
            int status =
                    ::gsl_interp2d_eval_e(parent._interp.get(), parent._wavelengths.getData(),
                                          parent._radii.getData(), parent._throughput.getData(), wavelength,
                                          _radius, _radiusAccel.get(), _wavelengthAccel.get(), &result);
            LSST_CHECK_GSL(pex::exceptions::RuntimeError, status);
            return result;
        }
...
public:
    InterpolatedTransmissionCurve(Impl impl, std::pair<double, double> throughputAtBounds)
            : _atBounds(throughputAtBounds), _impl(std::move(impl)) {
    ...
    std::pair<double, double> getWavelengthBounds() const override { return _impl.getWavelengthBounds(); }

    std::pair<double, double> getThroughputAtBounds() const override { return _atBounds; }

    void sampleAt(lsst::geom::Point2D const& point, ndarray::Array<double const, 1, 1> const& wavelengths,
                  ndarray::Array<double, 1, 1> const& out) const override {
        LSST_THROW_IF_NE(wavelengths.getSize<0>(), out.getSize<0>(), pex::exceptions::LengthError,
...
protected:
    ...
    private:
        PersistenceHelper()
                : schema(),
                  throughputAtMin(
                          schema.addField<double>("throughputAtMin", "throughput below minimum wavelength")),
                  throughputAtMax(
...
public:
    ...
    ProductTransmissionCurve(std::shared_ptr<TransmissionCurve const> a,
    ...
    std::pair<double, double> getWavelengthBounds() const override {
        ...
        auto determineWavelengthBound = [](double aWavelength, double bWavelength, double aThroughput,
                                           double bThroughput, auto isFirstOuter) -> double {
        ...
        return std::make_pair(determineWavelengthBound(aWavelengthBounds.first, bWavelengthBounds.first,
    ...
    std::pair<double, double> getThroughputAtBounds() const override {
        ...
        return std::make_pair(aAtBounds.first * bAtBounds.first, aAtBounds.second * bAtBounds.second);
    }

    void sampleAt(lsst::geom::Point2D const& position, ndarray::Array<double const, 1, 1> const& wavelengths,
                  ndarray::Array<double, 1, 1> const& out) const override {
...
public:
    ...
    TransformedTransmissionCurve(std::shared_ptr<TransmissionCurve const> nested,
                                 std::shared_ptr<geom::TransformPoint2ToPoint2> transform)
            : _nested(std::move(nested)), _transform(std::move(transform)) {}

    std::pair<double, double> getWavelengthBounds() const override { return _nested->getWavelengthBounds(); }
...
protected:
    ...
    std::shared_ptr<TransmissionCurve const> _transformedByImpl(
            std::shared_ptr<geom::TransformPoint2ToPoint2> transform) const override {
        return std::make_shared<TransformedTransmissionCurve>(_nested, transform->then(*_transform));
    }
...
std::shared_ptr<TransmissionCurve const> TransmissionCurve::makeSpatiallyConstant(
        ndarray::Array<double const, 1> const& throughput, ndarray::Array<double const, 1> const& wavelengths,
...
std::shared_ptr<TransmissionCurve const> TransmissionCurve::makeRadial(
        ndarray::Array<double const, 2> const& throughput, ndarray::Array<double const, 1> const& wavelengths,
        ndarray::Array<double const, 1> const& radii, double throughputAtMin, double throughputAtMax) {
    ::gsl_set_error_handler_off();
    LSST_THROW_IF_NE(
...
std::shared_ptr<TransmissionCurve const> TransmissionCurve::multipliedBy(
        TransmissionCurve const& other) const {
...
ndarray::Array<double, 1, 1> TransmissionCurve::sampleAt(
        lsst::geom::Point2D const& position, ndarray::Array<double const, 1, 1> const& wavelengths) const {
...
std::string TransmissionCurve::getPythonModule() const { return "lsst.afw.image"; }
