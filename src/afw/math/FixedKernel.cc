...
template std::shared_ptr<math::FixedKernel> table::io::PersistableFacade<math::FixedKernel>::dynamicCast(
...
FixedKernel::FixedKernel(Kernel const& kernel, lsst::geom::Point2D const& pos)
...
std::shared_ptr<Kernel> FixedKernel::resized(int width, int height) const {
    ...
    lsst::geom::Box2I bboxIntersect = getBBox();
...
public:
    std::shared_ptr<afw::table::io::Persistable> read(InputArchive const& archive,
        ...
        image::Image<Pixel> image(lsst::geom::Extent2I(record.get(keys.dimensions)));
