...
std::vector<std::shared_ptr<afw::detection::Footprint>> findCosmicRays(
        afw::image::MaskedImage<T> &mimage,               ///< Image to search
    ...
    typedef typename afw::image::MaskedImage<T>::Image ImageT;
    typedef typename ImageT::Pixel ImagePixel;
    typedef typename afw::image::MaskedImage<T>::Mask::Pixel MaskPixel;
    ...
    for (int j = 1; j < nrow - 1; ++j) {
        typename afw::image::MaskedImage<T>::xy_locator loc = mimage.xy_at(1, j);  // locator for data

        for (int i = 1; i < ncol - 1; ++i, ++loc.x()) {
            ImagePixel corr = 0;
            if (!is_cr_pixel<afw::image::MaskedImage<T> >(&corr, loc, minSigma, thresH, thresV, thresD, bkgd, cond3Fac)) {
                continue;
            }
            /*
             * condition #4
...
//    m.def("findCosmicRays", &findCosmicRays<float>);
//    m.def("findCosmicRays", &findCosmicRays<double>);
//}
// \endcond
}  // namespace algorithms
}  // namespace meas
